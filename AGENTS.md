...
The cross-board personal inbox lives at `(project)/my-work`, rendered as a **single-viewport board of to-do notes** (no page scroll on ≥lg; each cell scrolls internally). No search, no filter chips. The page is one grid (`page.tsx` → `PageShell`):

- **One white container (left):** `TaskNote` holds everything — the greeting band (`MyWorkGreeting`: date, greeting, "เหลืออีก N งาน" + progress bar, outlined stat tiles), then the tab bar, then the project-grouped list. Bands are separated by `slate-200` rules, never by tinted fills. The วันนี้ / เลยกำหนด tiles are shortcuts to the matching tab; the open tab's tile gets the `primary` ring.
- **Color split (60/30/10, no gradients):** 60 = white container + rail; 30 = `bg-slate-50` canvas, `slate-200` rules/outlines; 10 = `primary` (today count, progress fill, open-tab underline). `danger` only on overdue numbers.
- **Tabs (inside the container):** **"วันนี้ต้องทำ"** is the primary tab (session progress meter) and **"เลยกำหนด"** as a secondary tab — overdue is near-empty in real use, so it must not take half the screen; its count stays visible on the tab. Rows sort by priority (High → Low → none), then due date, and are grouped per project with the same `ProjectGroupedList` boxes as the rail (`withIcon` shows the project glyph; the row then drops its own project name). Tasks are ticked off in place via the row checkbox (`handleComplete`, undo toast). Red stays on the overdue count badge and the "เลย N วัน" text only. The page canvas is `bg-slate-50` (design.md `background`) so the white container lifts off it.
- **Right rail:** a full-height white column (not a card) with "กำหนดส่งที่จะถึง" (or "ไม่มีคิว" rows) and "ไม่มีวันที่", each grouped per project by `ProjectGroupedList`.

`(project)/today` no longer exists — `/today` redirects to `/my-work` (`next.config.ts`). Below `lg` the cells stack and the page scrolls.

> The Today note shows tasks **due today** only. The design also surfaces overdue items *into* Today ("ยกมาจากเลย N วัน"), which needs a plan/pin-for-today field the backend doesn't have yet — add that column + endpoint before implementing roll-over.
...
- **Reuse, don't fork components.** The page composes `MyWorkGreeting`, `MyWorkStatCards`, `DashboardGrid`, and `MyWorkSkeleton`; `DashboardGrid` places its cells into the page grid (`TaskNote`s + rail). Every task row is `CompactRow`: the default variant is a note line (tick-off checkbox, title, project dot + name, status, then aligned columns: due text unless the row is under Today, a High/Medium/Low priority chip with a `priority.*` disc, estimate when > 0); `rail` is the light tree row (title + one due/status text). Due labels are Thai (`วันนี้` / `เลย N วัน`). Snooze lives in `MyWorkTaskModal`. Don't duplicate row UI.
